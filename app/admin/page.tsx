...
function AdminHomeContent() {
  ...
  const loadBrands = useCallback(async () => {
    ...
    setLoading(true);
    // code 컬럼이 아직 없는 DB에서도 죽지 않도록 실패하면 그 컬럼 빼고 재시도
    let { data, error } = await supabase.from("brands").select("id,name,code,access_code").order("id", { ascending: true });
    if (error) {
      const retry = await supabase.from("brands").select("id,name,access_code").order("id", { ascending: true });
      data = (retry.data ?? []).map((b: any) => ({ ...b, code: null }));
    }
    setBrands(data ?? []);
    setLoading(false);
  }, []);
