...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error: updErr } = await supabaseAdmin.from("brands").update(update).eq("id", params.id).select().single();
  let data2 = data;
  let error = updErr;
  if (error && "code" in update) {
    delete update.code;
    ({ data: data2, error } = await supabaseAdmin.from("brands").update(update).eq("id", params.id).select().single());
  }

  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ brand: data2 });
}
